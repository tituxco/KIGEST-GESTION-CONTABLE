...
Public Class recibos
    ...
    Private Sub cargarInfoPer()
        ...
        Try
            ...
            conexionEmp.ChangeDatabase(EmpDB)
            sql.Connection = conexionEmp
            cadenabd = EmpDB & ".sdo_personal as per," & database & ".cm_sdo_convenios as cen, " & database & ".cm_sdo_jornada as jor," _
    ...
    Private Sub CargarHistorial()
        ...
        Try
            ...
            Dim consulta As New MySql.Data.MySqlClient.MySqlDataAdapter("select id as NumRec, concat(periodo_pago, ' ', mes, ' ', ano) as Periodo from sdo_recibos where idpersonal=" & Idpersonal & " order by id desc " & limite, conexionEmp)
            ...
            ElseIf meses < 6 And lblmodocont.Text = "QUINCENAL" Then
                Dim consultaQUI As New MySql.Data.MySqlClient.MySqlDataAdapter("select mes, sum(format(total_remunerativo,3)) as suma from sdo_recibos where periodo_pago <> 'AGUINALDO' and idpersonal=" & Idpersonal & " group by mes limit 0,12", conexionEmp)
                ...
                End If
            ElseIf meses >= 6 And lblmodocont.Text = "QUINCENAL" Then
                Dim consultaQUI As New MySql.Data.MySqlClient.MySqlDataAdapter("select mes, sum(format(total_remunerativo,3)) as suma from sdo_recibos where periodo_pago <> 'AGUINALDO' and idpersonal=" & Idpersonal & " group by mes", conexionEmp)
                Dim tablaQUI As New DataTable
                consultaQUI.Fill(tablaQUI)
                Dim maximo As Object = tablaQUI.Compute("MAX(suma)", "")
                If Not IsDBNull(maximo) Then
    ...
    Private Sub dtrecibos_UserDeletingRow(sender As Object, e As DataGridViewRowCancelEventArgs) Handles dtrecibos.UserDeletingRow
        ...
        If MsgBox("Esta seguro que desea eliminar este recibo?", vbYesNo + vbQuestion) = vbYes Then
            Try
                ...
                Dim comandoadd2 As New MySql.Data.MySqlClient.MySqlCommand(sqlQuery, conexionEmp)
    ...
    Private Sub Button7_Click(sender As Object, e As EventArgs) Handles Button7.Click
        Try
            ...
            from sdo_items_recibos where idrecibo=" & dtrecibos.CurrentRow.Cells(0).Value & " order by id asc", conexionEmp)
